            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="text-center mb-12">
                <h2 className="text-3xl font-bold text-gray-900 mb-4">
                  {locale === 'zh' ? '值得信赖的专业平台' : 'Trusted Professional Platform'}
                </h2>
                <p className="text-xl text-gray-600">
                  {locale === 'zh' ? '基于科学研究，获得用户认可' : 'Based on scientific research, trusted by users'}
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
                <div className="bg-white rounded-xl p-6 shadow-sm">
                  ...
                  <div className="text-gray-600">{locale === 'zh' ? '活跃用户' : 'Active Users'}</div>
                </div>
                <div className="bg-white rounded-xl p-6 shadow-sm">
                ...
                <div className="bg-white rounded-xl p-6 shadow-sm">
                  ...
                  <div className="text-gray-600">{locale === 'zh' ? 'PDF资源' : 'PDF Resources'}</div>
                </div>
                <div className="bg-white rounded-xl p-6 shadow-sm">
                  <div className="text-3xl font-bold text-green-600 mb-2">98%</div>
                  <div className="text-gray-600">{locale === 'zh' ? '用户满意度' : 'User Satisfaction'}</div>
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="py-20 bg-white" aria-labelledby="faq-heading">
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
              <header className="text-center mb-16">
                ...
                <p className="text-xl text-gray-600">
                  {locale === 'zh' ? '关于痛经缓解和月经健康的专业解答' : 'Professional answers about period pain relief and menstrual health'}
                </p>
              </header>
              ...
              <div className="space-y-6">
                <details className="bg-gray-50 rounded-lg p-6 group">
                  <summary className="font-semibold text-lg text-gray-900 cursor-pointer list-none flex items-center justify-between">
                    <span>{locale === 'zh' ? '痛经怎么缓解最快方法？' : 'What are the fastest ways to relieve menstrual cramps?'}</span>
                  ...
                  <div className="mt-4 text-gray-600 leading-relaxed">
                    {locale === 'zh' 
                      ? '最快的痛经缓解方法包括：1) 热敷下腹部和腰部；2) 轻柔按摩腹部；3) 适量运动如瑜伽；4) 服用布洛芬等非甾体抗炎药；5) 保持充足休息。建议结合多种方法，效果更佳。'
                      : 'The fastest menstrual cramp relief methods include: 1) Heat therapy on lower abdomen and back; 2) Gentle abdominal massage; 3) Light exercise like yoga; 4) NSAIDs like ibuprofen; 5) Adequate rest. Combining multiple methods works best.'
                    }
                  </div>
                </details>
                ...
                <details className="bg-gray-50 rounded-lg p-6 group">
                  <summary className="font-semibold text-lg text-gray-900 cursor-pointer list-none flex items-center justify-between">
                    <span>{locale === 'zh' ? '月经周期多少天算正常？' : 'What is considered a normal menstrual cycle?'}</span>
                    <svg className="w-5 h-5 text-gray-500 group-open:rotate-180 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                    </svg>
                  </summary>
                  <div className="mt-4 text-gray-600 leading-relaxed">
                    {locale === 'zh'
                      ? '正常的月经周期为21-35天，平均28天。月经持续时间通常为3-7天。如果周期长期不规律或有明显变化，建议咨询妇科医生。'
                      : 'A normal menstrual cycle ranges from 21-35 days, with an average of 28 days. Menstruation typically lasts 3-7 days. Consult a gynecologist if cycles are consistently irregular or show significant changes.'
                    }
                  </div>
                </details>
                ...
                <details className="bg-gray-50 rounded-lg p-6 group">
                  <summary className="font-semibold text-lg text-gray-900 cursor-pointer list-none flex items-center justify-between">
                    <span>{locale === 'zh' ? '中医如何调理月经不调？' : 'How does Traditional Chinese Medicine treat irregular periods?'}</span>
                    <svg className="w-5 h-5 text-gray-500 group-open:rotate-180 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                    </svg>
                  </summary>
                  <div className="mt-4 text-gray-600 leading-relaxed">
                    {locale === 'zh'
                      ? '中医通过辨证论治调理月经：1) 针灸调节气血；2) 中药方剂如四物汤、逍遥散；3) 食疗调养；4) 生活方式调整。需要专业中医师诊断体质后制定个性化方案。'
                      : 'TCM treats irregular periods through: 1) Acupuncture to regulate qi and blood; 2) Herbal formulas like Si Wu Tang; 3) Dietary therapy; 4) Lifestyle adjustments. Professional TCM diagnosis is needed for personalized treatment.'
                    }
                  </div>
                </details>
              </div>
